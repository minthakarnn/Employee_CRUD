document.addEventListener("turbo:load", () => {
  ...
  function renderEmployeeTable(employees) {
    ...
    if (employees.length === 0) {
      ...
      cell.colSpan = 6; // เพิ่มจำนวนคอลัมน์ที่ต้องการให้ขยาย
    ...
    employees.forEach((employee, index) => {
      ...
      row.innerHTML = `
        ...
        <td class="text-center"><button class="btn-success" onclick="showEmployee(${employee.id})">Show</button></td>
        <td class="text-center"><button class="btn-primary" onclick="editEmployee(${employee.id})">Edit</button></td>
        <td class="text-center"><button class="btn-danger" onclick="deleteEmployee(${employee.id})">Delete</button></td>
      `;
    });
  }
  ...
  if (confirm("Are you sure you want to delete this employee?")) {
    ...
    .then(response => {
      if (response.ok) {
        ...
        // เรียกใช้งานฟังก์ชันดึงข้อมูลใหม่ทันทีหลังจากลบสำเร็จ
